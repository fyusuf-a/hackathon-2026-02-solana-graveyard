...
use crate::{errors::AuctionError, state::{Auction, Referrers}};
...
impl<'info> Bid<'info> {
    ...
    fn compute_distribution(&mut self, lamports: u64, is_referred: bool) -> DistributionScheme {
        if self.auction.referral_structure.is_some() && is_referred {
            ...
            let fee = lamports.checked_mul(structure.base_fee_bps as u64).unwrap_or(0)
                .checked_div(10_000).unwrap_or(0);
            let discount = lamports.checked_mul(structure.buyer_discount_bps as u64).unwrap_or(0)
                .checked_div(10_000).unwrap_or(0);
